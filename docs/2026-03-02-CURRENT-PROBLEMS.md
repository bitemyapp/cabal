...
- It's highly likely that I should create an example-problems (not `examples/`, that's a thing in `cargo`) directory with some fugazi test cases that are meant to resolve/achieve unanimity quickly for cheaper and quicker testing. End-to-end automated tests would be even better.

- I'm strongly considering making private evals aimed at evaluating model performance on specific kinds of diagnostic and analysis problems that are often very difficult, especially for anything other than Codex 5.3 and Opus 4.6.
